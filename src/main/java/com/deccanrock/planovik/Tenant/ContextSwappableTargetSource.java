...
import com.deccanrock.planovik.Tenant.TargetRegistry;
import com.deccanrock.planovik.Tenant.BeanFactoryTargetRegistry;
...
public class ContextSwappableTargetSource implements TargetSource, InitializingBean, ApplicationContextAware {
    ...
    public Object getTarget() throws Exception {
        final TenantEntity contextName = ContextHolder.getTenant();
     ...
     */
    private Object getTarget(final TenantEntity contextName) {
        Object target = this.resolveTarget(contextName);
        if (target == null && this.alwaysReturnTarget) {
            this.logger.debug("Return default target for context '{}'", contextName);
            target = this.defaultTarget;
        }
        return target;
    }

    protected Object resolveTarget(final TenantEntity contextName) {
        Object target = null;
        for (final TargetRegistry<?> registry : this.registries) {
            this.logger.debug("Using '{}' to lookup '{}'.", registry, contextName);
            target = registry.getTarget(contextName);
            if (target != null) {
                break;
            }
        }
