							...
							<h4 class="widget-title">
		                    	<c:choose>
		                    		<c:when test="${user.mode == 'Create'}">
										<span>Create Service: ${serviceprovider.typestr} - <i>${serviceprovider.name}</i></span>
		                        	</c:when>
		                        	<c:otherwise>
		                        		<span>Edit Service: ${serviceprovider.typestr} - <i>${serviceprovider.name}</i></span>
		                        	</c:otherwise>	
		                    	</c:choose>		                        							
							</h4>
						</div>				
       								...
       								<div class="row">
					                	<div class='col-xs-1'>       							
							                <div class="form-group">
							                    <div class="clearfix">
							                    	<label for="travelmodelist">Service Name</label>
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.name}">
						                        			<form:input type="text" path="name" id="name" class="col-xs-9 col-sm-7"  
					                	...
					                	<div class='col-xs-1'>       							
							                <div class="form-group">
							                    <div class="clearfix">
							                    	<label for="travelmodelist">City</label>
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.city}">
						                        			<form:input type="text" path="city" id="city" class="col-xs-9 col-sm-7"  
					                	...
					                	<div class='col-xs-1'>       							
							                <div class="form-group">
							                    <div class="clearfix">
							                    	<label for="travelmodelist">Country</label>
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.country}">
						                        			<form:input type="text" path="country" id="country" class="col-xs-9 col-sm-7"  
						                        			value="${serviceprovider.country}" />
						                        	    </c:when>
														<c:otherwise>
						                    				<form:input type="text" path="country" id="country" class="col-xs-9 col-sm-7" />                                	
														</c:otherwise>
									...
									<c:if test="${serviceprovider.type == 0}">					
       								<div class="row">					                
					                    		...
					                    		<div class="clearfix">
						                    		...
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.iatacode}">
						                        			<form:input type="text" path="iatacode" id="iatacode" class="col-xs-9 col-sm-7"  
						                        			value="${serviceprovider.iatacode}" />
						                        	    </c:when>
														<c:otherwise>
						                    				<form:input type="text" path="iatacode" id="iatacode" class="col-xs-9 col-sm-7" />                                	
														</c:otherwise>
						                    		...
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.fullservice}">
		                									<c:if test="${serviceprovider.fullservice == 0}">
																<label>
																	<form:input  name="fullservice" id="fullservice" path="fullservice" class="ace ace-switch ace-switch-2" type="checkbox" />
																	<span class="lbl"></span>
																</label>
															</c:if>
		                									<c:if test="${serviceprovider.fullservice == '1'}">
																<label>
																	<form:input name="fullservice" id="fullservice" path="fullservice" class="ace ace-switch ace-switch-2" type="checkbox" checked />
																	<span class="lbl"></span>
																</label>
															</c:if>
														</c:when>
					                	...
					                	<div class='col-xs-1'>
					                		<div class="form-group">
					                    		<div class="clearfix">
													<label for = "fullservice">Domestic only</label>
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.domesticonly}">
		                									<c:if test="${serviceprovider.domesticonly == 0}">
																<label>
																	<input  name="domesticonly" id="domesticonly" class="ace ace-switch ace-switch-2" type="checkbox" />
																	<span class="lbl"></span>
																</label>
															</c:if>
		                									<c:if test="${serviceprovider.domesticonly == '1'}">
																<label>
																	<input name="domesticonly" id="domesticonly" class="ace ace-switch ace-switch-2" type="checkbox" checked />
																	<span class="lbl"></span>
																</label>
															</c:if>
														</c:when>
       								...
       								<div class="row">
					                	<div class='col-xs-3'>
					                		<div class="control-group">
						                    		<label for = "email">Coverage</label>
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.coverage}">						                    			
															<div class="radio">
															<c:if test="${serviceprovider.coverage == 0}">
																<label>
																	<form:radiobutton path="coverage" name="coverage" value="0" selected class="ace" />
																	<c:/if>
																	<span class="lbl"> City Transfers</span>
																</label>
																<label>
															...
															<c:if test="${serviceprovider.coverage == 1}">
																...
																<label>
																	<form:radiobutton path="coverage" name="coverage" value="1" selected class="ace" />
																	<span class="lbl"> City and inter city</span>
																</label>
															</c:if>
															</div>
       								...
       								<div class="row">
					                	<div class='col-xs-3'>
					                		<div class="control-group">
					                    		...
					                    		<c:choose>
					                    			<c:when test="${not empty serviceprovider.desc}">						                    			
														<form:textarea class="form-control limited" id="desc" name="desc" rows="1" path="desc" 
														value="${serviceprovider.desc}" maxlength="200" style="width:94%;"/>					         
					                        		</c:when>
					                        	</c:choose>
												<c:otherwise>
													<form:textarea class="form-control limited" id="desc" name="desc" rows="1" path="desc" 
													maxlength="200" style="width:94%;"/>					         
												</c:otherwise>
									</c:if>
							                    ...
							                    <div class="clearfix">
						                    		...
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.contactname}">
						                        			<form:input type="text" path="contactname" id="contactname" class="col-xs-9 col-sm-7"  
							                    ...
							                    <div class="clearfix">
						                    		...
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.contactphone}">
						                        			<form:input type="text" path="contactphone" id="contactphone" class="col-xs-9 col-sm-7"  
						                        			value="${serviceprovider.contactphone}" />
						                        	    </c:when>
														<c:otherwise>
						                    				<form:input type="text" path="contactphone" id="contactphone" class="col-xs-9 col-sm-7" />                                	
														</c:otherwise>
							                    ...
							                    <div class="clearfix">
						                    		...
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.contactemail}">
						                        			<form:input type="text" path="contactemail" id="contactemail" class="col-xs-9 col-sm-7"  
						                        			value="${serviceprovider.contactemail}" />
						                        	    </c:when>
														<c:otherwise>
						                    				<form:input type="text" path="contactemail" id="contactemail" class="col-xs-9 col-sm-7" />                                	
														</c:otherwise>
							                    ...
							                    <div class="clearfix">
						                    		...
						                    		<c:choose>
						                    			<c:when test="${not empty serviceprovider.contactwebsite}">
						                        			<form:input type="text" path="contactwebsite" id="contactwebsite" class="col-xs-9 col-sm-7"  
						                        			value="${serviceprovider.contactwebsite}" />
						                        	    </c:when>
														<c:otherwise>
						                    				<form:input type="text" path="contactwebsite" id="contactwebsite" class="col-xs-9 col-sm-7" />                                	
														</c:otherwise>
